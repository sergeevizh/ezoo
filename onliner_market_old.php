...
$simpla->db->query("SELECT v.price, v.compare_price, v.id AS variant_id, p.name AS product_name, p.name_yan_market AS name_market, v.name AS variant_name, v.position AS variant_position, v.sku AS variant_sku, p.id AS product_id, p.url, p.annotation, p.body, pc.category_id, cat.name_cat_market_onliner AS name_cat_market_onliner, cat.name AS name_cat, i.filename AS image, b.name AS brand, b.clear_name AS brand_clear_name, b.manufacturer AS manufacturer, b.importer AS importer
					FROM __variants v LEFT JOIN __products p ON v.product_id=p.id
					LEFT JOIN s_brands b ON b.id = p.brand_id
					LEFT JOIN __products_categories pc ON p.id = pc.product_id AND pc.position=(SELECT MIN(position) FROM __products_categories WHERE product_id=p.id LIMIT 1)
...
foreach ($products as $p) {
    ...
    $comment->appendChild($dom->createTextNode('Отличные цены, принимаем карты рассрочки. Большой выбор кормов, лакомств, наполнителей, переносок и других зоотоваров. Профессиональная консультация и помощь в подборе. Аккуратные курьеры. Бесплатная доставка от 19 руб по г. Минск и 10 км за МКАД.'));
