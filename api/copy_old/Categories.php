...
class Categories extends Simpla
{
    ...
    public function delete_category($ids)
    {
        ...
        foreach ($ids as $id) {
            ...
            if (!empty($category->children)) {
                ...
                $this->db->query($query);
                $query = $this->db->placehold("DELETE FROM __related_categories WHERE category_id IN( ?@ ) OR related_id IN( ?@ ) LIMIT 1", $category->children, $category->children);
                $this->db->query($query);
            }
        }
        unset($this->categories_tree);
    ...
    public function get_related_categories($category_id = array())
    {
        if (empty($category_id)) {
            return array();
        }

        $category_id_filter = $this->db->placehold('AND category_id in(?@)', (array)$category_id);

        $query = $this->db->placehold("SELECT category_id, related_id, position
					FROM __related_categories
					WHERE
					1
					$category_id_filter
					ORDER BY position
    ...
    public function add_related_category($category_id, $related_id, $position = 0)
    {
        $query = $this->db->placehold("INSERT IGNORE INTO __related_categories SET category_id=?, related_id=?, position=?", $category_id, $related_id, $position);
        $this->db->query($query);
        return $related_id;
    }

    public function delete_related_category($category_id, $related_id)
    {
        $query = $this->db->placehold("DELETE FROM __related_categories WHERE category_id=? AND related_id=? LIMIT 1", intval($category_id), intval($related_id));
        $this->db->query($query);
    }
    ...
    private function init_categories($filter = array())
    {
        ...
        $query = $this->db->placehold("SELECT c.id, c.parent_id, c.name, c.description, c.url, c.meta_title, c.meta_keywords, c.meta_description, c.image, c.visible, c.visible_is_main,c.visible_childs, c.position$select_products_count
										FROM __categories c
										$left_join_products_count
										$group_by
        ...
        //                               FROM __categories c L
        //EFT JOIN __products_categories pc ON pc.category_id=c.id
        // LEFT JOIN __products p ON p.id=pc.product_id AND p.visible
        // GROUP BY c.id ORDER BY c.parent_id, c.position");
        ...
        while (!empty($categories)  && !$finish) {
            ...
            foreach ($categories as $k=>$category) {
                ...
                if (isset($filter['visible']) && intval($filter['visible']) != $category->visible) {
                    continue;
                }
