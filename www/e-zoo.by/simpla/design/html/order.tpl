...
<div id="name">
	...
	<a href="{url module=OrderAdmin id=$order->id view=export}" style="margin-left: 20px;"><img src="./design/images/excel.svg" style="width: 32px;height: 32px;" title="Экспорт заказа"></a>
	<span style="margin-left: 20px;">
		{if $order->call_back}
			<img src='design/images/phone-call--green.svg' alt='Перезвонить' title='Перезвонить' style="width: 32px;height: 32px;">
		{else}
			<img src='design/images/phone-call.svg' alt='Не перезванивать' title='Не перезванивать' style="width: 32px;height: 32px;">
		{/if}
	</span>
...
{/if}
	...
	{if $success_export}
	<div class="message message_success">
		<span class="text">Заказ сохранен в формате .xls</span>
		<a href="{$config->root_url}/{$success_export}" target="_blank" download>Скачать файл</a>
	</div>
	{/if}
...
<div id="order_details">
	...
	<ul class="order_details">
		...
		<li>
			<label class=property>Дата и время</label>
			<div class="edit_order_detail" style='display:none;'>
				<input name="self_discharge_time" class="simpla_inp" type="text" value="{$order->self_discharge_time|escape}" />
			</div>
			<div class="view_order_detail">
				{$order->self_discharge_time|escape}
			</div>
		</li>
		<li>
		...
		<li>
			<label class=property>Квартира</label>
			<div class="edit_order_detail" style='display:none;'>
				<input name="flat_num" class="simpla_inp" type="text" value="{$order->flat_num|escape}" />
			</div>
			<div class="view_order_detail">
				{$order->flat_num|escape}
			</div>
		</li>
		<li>
		...
		<li>
			<label class=property>Промокод</label>
			<div class="edit_order_detail" style='display:none;'>
				<textarea name="promo">{$order->promo|escape}</textarea>
			</div>
			<div class="view_order_detail">
	...
	<ul>
		{foreach $labels as $l}
			{if $l->id != 7 || in_array('check_packed', $manager->permissions) }
				<li>
	...
	<div class='layer'>
		<h2>Примечание Оператор<a href='#' class="edit_note"><img src='design/images/pencil.png' alt='Редактировать' title='Редактировать'></a></h2>
		<ul class="order_details">
			<li>
	...
	<div class='layer'>
		<h2>Примечание Склад<a href='#' class="edit_wh_note"><img src='design/images/pencil.png' alt='Редактировать' title='Редактировать'></a></h2>
		<ul class="order_details">
			<li>
				<div class="edit_wh_note" style='display:none;'>
					<label class=property>Ваше примечание (не видно пользователю)</label>
					<textarea name="wh_note">{$order->wh_note|escape}</textarea>
				</div>
				<div class="view_wh_note" {if !$order->wh_note}style='display:none;'{/if}>
					<label class=property>Ваше примечание (не видно пользователю)</label>
					<div class="note_text">{$order->wh_note|escape}</div>
				</div>
			</li>
		</ul>
	</div>
...
<div id="purchases">
	...
	<div id="list" class="purchases">
		...
		<div class="row {if $purchase->product->marketing_offer}yellow{/if}">
			...
			<div class="purchase_name cell">
				...
				{else}
				{if $purchase->product}
				<a class="related_product_name" style="color: #00afee" href="index.php?module=ProductAdmin&id={$purchase->product->id}&return={$smarty.server.REQUEST_URI|urlencode}">{$purchase->product_name}</a>
				{else}
				{$purchase->product_name}
				{/if}
				{/if}
...
*}
	<div class="block discount layer">
		<h2>Скидка{if $order->coupon_code} ({$order->coupon_code}){/if}</h2>
		<input type=text name=coupon_discount value='{$order->coupon_discount}' style="background: transparent; border: none; color: #333333; text-align: right" onmousedown="(function(e){ e.preventDefault(); })(event, this)"> <span class=currency>{$currency->sign}</span>
	</div>
	<div class="block custom-discount layer">
		<h2>Ввести скидку вручную</h2>
		<input name=custom_discount value='1' type="checkbox" id="custom_discount" {if $order->custom_discount}checked{/if}/> <label for="custom_discount">Разрешить вводить скидку вручную</label>
		<input type=text name=custom_discount_val> <span class=currency>{$currency->sign}</span>
	</div>

	<div class="subtotal layer">
	С учетом скидки<b> {($subtotal-$subtotal*$order->discount/100-$order->coupon_discount)|round:2} {$currency->sign}</b>
	</div>
	...
	<div class="block delivery">
		...
		<select name="courier_id">
			...
			{foreach $couriers as $courier}
				<option value="{$courier->id}" {if $courier->id==$courier_id}selected{/if}>{$courier->name}</option>
			{/foreach}
		</select>
	</div>
...
<script>
	var isAdmin = {if in_array('uncheck_packed', $manager->permissions)} true {else} false {/if};
</script>
...
<script>
$(function() {
	if (!isAdmin && $('#label_7').prop('checked')) {
		$('#label_7').click(function (e) {
			e.preventDefault()
		})

		$('#label_7').parent('label').click(function (e) {
			e.preventDefault()
		})
	}
  ...
  function change_variant(element)
  {
	...
	// Редактировать примечание склад
	$("div#order_details a.edit_wh_note").click(function() {
		$("div.view_wh_note").hide();
		$("div.edit_wh_note").show();
		return false;
	});
...
.autocomplete-suggestions .autocomplete-suggestion{cursor: default; border-bottom: 1px solid #d0d0d0}
