...
{capture name=tabs}
	...
	<li><a href="{url module=OrdersDeliveryArea keyword=null id=null page=null label=null}">Области доставки</a></li>
	{/if}
{/capture}
...
<div id="header">
	...
	{if $status==1 || $status==0}
		{if $filter_day==false}
			<a class="filter-link" href="/simpla/index.php?module=OrdersAdmin&status={$status}&now_day=Y">Включить фильтр</a>
		{else}
			<a class="filter-link" href="/simpla/index.php?module=OrdersAdmin&status={$status}">Отключить фильтр</a>
		{/if}
	{/if}
</div>
...
<div id="main_list">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		<div id="list">
			{$old_date = $order->self_discharge_time}
			{foreach $orders as $order}
				{if $status==1 || $status==0}
					{if $filter_day==true}
						{if $old_date!=$order->self_discharge_time}
						<div class="row" style="text-align:center; color:red;padding: 5px">{$order->self_discharge_time}</div>
						{/if}
						{$old_date = $order->self_discharge_time}
					{/if}
				{/if}
<!--			TODO blue - если заказ помечен ветпрепарат-->
			<div class="{if $order->paid}green{/if} row {if $order->lecense}blue{/if} {if $order->marketing_offer}yellow{/if}">
				...
				<div class="order_name cell">
					...
					{if $order->note}
						<br>Примечание Оператор
						<div class="note">{$order->note|escape}</div>
					{/if}
					{if $order->wh_note}
						<br>Примечание Склад
						<div class="note">{$order->wh_note|escape}</div>
					{/if}
				</div>
				<div class="icons cell">
					<a href='{url module=OrderAdmin id=$order->id view=print}'  target="_blank" class="print" title="Печать заказа"></a>
					<a href='{url module=OrderAdmin id=$order->id view=export}'  target="_blank" class="excel" title="Экспорт заказа"></a>
					{if in_array('delete_orders', $manager->permissions)}<a href='#' class=delete title="Удалить"></a>{/if}
				</div>
				...
				<div class="icons cell">
					...
					{/if}
					{if $order->call_back}
						<img src='design/images/phone-call--green.svg' alt='Перезвонить' title='Перезвонить' style="width: 16px;height: 16px;">
					{else}
						<img src='design/images/phone-call.svg' alt='Не перезванивать' title='Не перезванивать' style="width: 16px;height: 16px;">
					{/if}
				</div>
				{if $keyword}
				<div class="icons cell">
		...
		<select name="action">
			...
			{/foreach}
			{if in_array('delete_orders', $manager->permissions)}
