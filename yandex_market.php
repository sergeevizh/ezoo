...
/*$simpla->db->query("SELECT offsetmy FROM __cron_offs");
$cron_offs = $simpla->db->results()[0]->offsetmy;
echo "<pre>";
print_r($cron_offs);
echo "</pre>";*/
// Товары
...
/*if (empty($products)) {
    $simpla->db->query("UPDATE __cron_offs SET `offsetmy` = 0");
    die();
}
else {
    $cron_offs= $cron_offs +100;
    $simpla->db->query("UPDATE __cron_offs SET `offsetmy` = ".$cron_offs);

}*/

    foreach ($products as $p) {
    $countProd++;
        $utm = '?utm_source=yandex.market&utm_medium=cpc';
        if ($p->cat_url){
            $utm .= '&utm_campaign=' . $p->cat_url;
        }

        if ($p->brand_url){
            $utm .= '&utm_content=' . $p->brand_url;
        }

        if ($p->variant_id){
            $utm .= '&utm_term=' . $p->variant_id;
        }
    ...
    $url->appendChild($dom->createTextNode('https://e-zoo.by/products/'.$p->url.$utm.$variant_url));
...
$dom->save(dirname(__FILE__).'/yandex_products_price.xml'); // сохранение файла
echo "Обработано ".$countProd.' товара';
echo '<br>Экспорт в файл выполнен';
echo '<br>Ссылка на файл : <a href="/yandex_products_price.xml" download>Yandex_products_price</a>';
