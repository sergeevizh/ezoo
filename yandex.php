...
// Товары
$simpla->db->query("SELECT v.price, v.id AS variant_id, p.name AS product_name, v.name AS variant_name, v.position AS variant_position, v.sku AS variant_sku, p.id AS product_id, p.url, p.annotation, p.body, pc.category_id, i.filename AS image, b.name AS brand
...
if($p->image) {
print "<picture>".$simpla->design->resize_modifier($p->image, 500, 500)."</picture>
";}
